  ...
  return (
    <div className='mt-3 text-center text-xl h-[72vh] flex flex-col justify-center items-center'>
      ...
      <motion.div className="shotsForm bg-white w-[20rem] md:w-[26rem] p-6 mt-10 rounded-xl font-berk-shwash" initial={{ opacity: 0 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.7 }}>
        <form onSubmit={handleSubmit} method="post" aria-label='Select shots form'>
          <label htmlFor="numShots" className="block mb-7 md:font-semibold text-foreground text-[1rem] md:text-[1.15rem] tracking-wide">Before entering the photobooth, please select the number of shots you want</label>
          <ShotsDropdown value={shots} onChange={setShots} />
          <br />
          <motion.button 
            type="submit" 
            className={`bg-foreground text-card text-[0.7rem] md:text-[1rem] py-2 px-5 rounded-xl md:rounded-2xl hover:bg-foreground/85 transition-colors ${!shots ? 'cursor-not-allowed' : 'cursor-pointer'}`}
            disabled={!shots}
            whileHover={shots && { scale: 1.05, transition: { ease: "easeInOut"} }}
          >
            Let's Take Photos!
          </motion.button>
        </form>
